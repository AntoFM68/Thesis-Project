...
# default_ac_hid = [512, 256]
# default_cr_hid = [512, 256]
SUCCESS = 1
FAIL = 0
SERIOUS_FAIL = -1
...
def get_params():
    ...
    # Currently Unused
    # parser.add_argument('--NORM_DIST',            type=int, default=1,                    help='Normalize (1) Linear Distance or not (0)')
    # parser.add_argument('--NORM_ANG',             type=int, default=1,                    help='Normalize (1) Angular Distance or not (0)')
...
#     parser = argparse.ArgumentParser(description='ReplayBuffer_Params')
    parser.add_argument('--MEMORY_CAPACITY',      type=int,   default=50000,        help='Capacity of Memory')
...
#     parser = argparse.ArgumentParser(description='Reward_Params')
    parser.add_argument('--DONE_REWARD',          type=float, default=30,           help='Reward given only if EPS_DIST is reached')
    parser.add_argument('--DIST',                 type=float, default=1,            help='Proportional for the distance component of the reward')
    parser.add_argument('--TRACK',                type=float, default=1,            help='Proportional for the distance component on track of the reward')
    ...
    # parser.add_argument('--SLIDE_PROP',           type=float, default=0,           help='Proportional for the sliding component of the reward')
    # parser.add_argument('--SPEED_BONUS',          type=float, default=0,           help='Proportional of speed bonus: higher speed is rewarded')
    # parser.add_argument('--ACTION_COST',          type=float, default=0,           help='Proportional of cost for taking an action (motors)')
